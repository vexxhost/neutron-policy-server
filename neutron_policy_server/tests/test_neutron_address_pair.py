...
class TestAddressPairCasesFlaskBase(test.TestCase, base_test.AllowedAddressPairDBTestCase):
    ...
    def setUp(self, plugin=None, ext_mgr=None):
        ...
        self.update_port_not_exist['target']['id'] = "52c5a95c-9310-4993-a731-89cfd5a41fd9"
...
class TestAddressPairCasesFlask(TestAddressPairCasesFlaskBase):
    ...
    def test_port_delete_success(self):
        response = self.client.post("/port-delete", json=self.delete_port_dep_json)  # pylint: disable=E1101
        self.assertEqual(b'True', response.data)
        self.assertEqual(200, response.status_code)

    def test_port_delete_fail_with_dep(self):
        response = self.client.post("/port-delete", json=self.delete_port_json)  # pylint: disable=E1101
        self.assertEqual(b'Address pairs dependency found for this port.', response.data)
        self.assertEqual(403, response.status_code)

    def test_port_update_success(self):
        response = self.client.post("/port-update", json=self.update_port_dep)  # pylint: disable=E1101
        self.assertEqual(b'True', response.data)
        self.assertEqual(200, response.status_code)

    def test_port_update_success_no_address_change(self):
        response = self.client.post("/port-update", json=self.update_port_no_address)  # pylint: disable=E1101
        self.assertEqual(b'True', response.data)
        self.assertEqual(200, response.status_code)

    def test_port_update_fail_no_match_port(self):
        response = self.client.post("/port-update", json=self.update_port_not_exist)  # pylint: disable=E1101
        self.assertEqual(b'No match port found.', response.data)
        self.assertEqual(403, response.status_code)

    def test_port_update_fail(self):
        response = self.client.post("/port-update", json=self.update_port)  # pylint: disable=E1101
        self.assertEqual(b'Address pairs dependency found for this port.', response.data)
        self.assertEqual(403, response.status_code)
